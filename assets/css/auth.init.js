define([
  ...
  var $button = angular.element(document.getElementById('signinButton'));
    ...
    gapi.auth.signIn({
      ...
      callback: function signinCallback(ar) {
        ...
        if (!ar.status.signed_in) {
          ...
          $button.off('click'); // this is a one-off initialisation
          ...
          */
          ...
          $button.find('span').html('Sign out of');
          $button.on('click', function() {
            gapi.auth.signOut();
            $button.removeClass('active');
            $button.find('span').html('Sign in to');
            $button.on('click', signinCallback);
          });
